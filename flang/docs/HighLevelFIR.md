...
operations in FIR to allow a simpler lowering to a higher-level FIR
representation that would later be lowered to the current FIR representation via
...
## Variable and Expression value concepts in FIR
...
## Adding an expression value concept in FIR
...
fir.elemental operation. The fir.expr<T> type is not intended to be used with
scalar types that already have SSA value types (e.g., integer or real scalars).
Instead, these existing SSA types will implicitly be considered as being
expressions when used in high-level FIR operations, which will simplify
interfacing with other dialects that define operations with these types (e.g.,
the arith dialect).

These fir.expr values could then be placed in memory when needed (assigned to a
variable, passed as a procedure argument, or an IO output item...) via
fir.assign or fir.associate operations that will later be described.

When no special optimization pass is run, a translation pass would lower the
operations producing fir.expr to buffer allocations and memory operations just
as in the currently generated FIR.

However, these high-level operations should allow the writing of optimization
passes combining chains of operations producing fir.expr into optimized forms
via pattern matching on the operand tree.

The fir.elemental operation will be discussed in more detail below. It allows
...
## Proposed changes to FIR

### New FIR Operations and Types

#### Introduce a fir.expr<T> type

Motivation: avoid the need to materialize expressions in temporaries while
lowering.

Syntax: ``` !fir.expr<[extent x]* T [, class]> ```

- `[extent x]*` represents the shape for arrays similarly to !fir.array<> type,
  except that the shape cannot be assumed rank (!fir.expr<..xT> is invalid).
...
examples: !fir.expr<fir.char<?>>, !fir.expr<10xi32>,
!fir.expr<?x10x?xfir.complex<4>>

T in scalar fir.expr<T> can be:
-   A character type (fir.char<10, kind>, fir.char<?, kind>)
-   A derived type: (fir.type<t{...}>)

T in an array fir.expr< e1 x ex2 ..  : T> can be:
...
polymorphic entities). The fir.expr type has an optional "class" flag to denote
this: fir.expr<T, class>.
...
-   What is the extent : via fir.get_extent %expr, dim
-   What are the length parameters: via fir.get_typeparam %expr [, param_name]
-   What is the dynamic type: via fir.get_dynamic_type %expr
...
-   %var = fir.associate %expr [attributes about the association]->
    AnyMemoryOrBoxType
-   fir.end_association %var

The intention is that the fir.expr<T> is the result of an operation, and should
...
describing the temporary. Clean-up for the temporary will be inserted after the
last use of the fir.expr. Note that, at least at first, lowering may help FIR to
find the last use of a fir.expr by explicitly inserting a fir.finalize %expr
operation that may turn into a no-op if the expression is not later materialized
in memory.
...
Syntax:
```
%var = fir.associate %expr_or_var {fir.def = mangled_uniq_name, attributes} (AnyExprOrVarType) -> AnyVarType
```

fir.associate is used to represent the following associations:
...
When the operand is a variable, fir.associate allows changing the attributes of
...
When the operand is an expression, fir.associate allows associating a storage
...
#### fir.end_association operation

Motivation: mark the place where some association should end and some side
effects might need to occur.

The fir.end_associate is a placeholder to later insert deallocation/finalization
if the variable was associated with an expression, and to insert
copy-out/deallocation if the variable was associated with another variable with
a copy-in.

Syntax:
```
fir.end_association %var [%original_variable] {fir.ref = var_mangled_name, attributes}
```
...
argument of the fir.associate that produced %var) must be provided. The
...
Syntax:
```
fir.finalize %var {fir.ref = var_mangled_name, attributes}
```
...
Syntax:
```
%var = fir.designate %base [“component”,] [(%i, %k:l%:%m)] [substr ub, lb] [imag|real] [shape extent1, extent2, ....] [lbs lb1, lb2, .....] [typeparams %l1, ...] {fir.ref = base_mangled_name, fir.def = mangled_name, attributes}
```

fir.designate is intended to encode a single part-ref (as defined by the
fortran standard). That means that a(:)%x(i, j, k) must be split into two
fir.designate: one for a(:), and one for x(i, j, k).  If the base is ranked, and
the component is an array, the subscripts are mandatory and must not contain
triplets. This ensures that the result of a fir.designator cannot be a
"super-array".

The subscripts passed to fir.designate must be based on the base lower bounds
(one by default).
...
fir.designate is not intended to describe vector subscripted variables.
Instead, lowering will have to introduce loops to do element by element
addressing. See the Examples section.  This helps keeping fir.designate simple,
...
reallocation logic in lowering already, duplicating the fact that fir.assign
should deal with it).
...
Syntax:
```
fir.ptr_assign %var [[reshape %reshape] | [lbounds %lb1, …., %lbn]] to %ptr
```

It is important to keep pointer assignment at a high-level so that they can
later correctly be processed in fir.forall.
...
fir.allocate has to be created for each element of the allocation-list.
Otherwise, it is a naive lowering of the ALLOCATE statement.

Syntax:
```
%stat = fir.allocate %var [%shape] [%type_params] [[src=%source] | [mold=%mold]] [errmsg =%errmsg]
```

#### fir.deallocate

Motivation: keep deallocation explicit in FIR, while allowing later lowering to
Fortran runtime calls to allow the runtime to do Fortran specific bookkeeping or
flagging of allocations.

Similarly to fir.allocate, one operation must be created for each
allocate-object-list object.

Syntax:
```
%stat = fir.deallocate %var [errmsg=err].
```
...
The fir.elemental operation can be seen as a closure: it is defining a function
of the indices that returns the value of the element of the represented array
expression at the given indices. This an operation with an MLIR region. It
allows detailing how an elemental expression is implemented at the element
level, without yet requiring materializing the operands and result in memory.
The fir.expr<T> elements value can be obtained using fir.apply.

The element result is built with a fir.result op, whose result type can be a
scalar fir.expr<T> or any scalar constant size types (e.g. i32, or f32).
...
between the fir.elemental and the fir.apply, the region of the fir.elemental can
be inlined at the fir.apply. Otherwise, if there is no such guarantee, or if the
fir.elemental is not “visible” (because its result is passed as a block
argument), the fir.elemental will be lowered to an array temporary. This will be
done as a FIR to FIR optimization pass. Note that MLIR inlining could be used if
fir.elemental implemented the CallableInterface and fir.apply the CallInterface.
But MLIR generic inlining is probably too generic for this case: no recursion is
possible here, the call graphs are trivial, and using MLIR inlining here could
introduce later conflicts or make normal function inlining more complex because
FIR inlining hooks would already be used.

fir.elemental allows delaying elemental array expression buffering and
...
- It avoids defining one operation per elemental operation or intrinsic,
  ...
  elemental regions. This avoids growing FIR and having to maintain about a
  hundred operations.
...
inlining of fir.elemental much easier.

If fir.elemental inlining is not performed, or if the fir.expr<T> array
expression is produced by another operation (like fir.intrinsic) that is not
rewritten, fir.apply will be lowered to an actual addressing operation that will
address the temporary that was created for the fir.expr<T> value that was
materialized in memory.

fir.apply indices will be one based to make further lowering simpler.

Syntax:
```
%element = fir.apply %array_expr %i, %j: (fir.expr<?x?xi32>) -> i32
```
...
fir.expr<T>. The arguments may be fir.expr<T>, simple scalar types (e.g., i32,
f32), or variables.
...
intrinsics, that will be implemented using fir.elemental + mlir math dialect
...
Without any optimization, codegen would then translate the operations to exactly
the same FIR as currently generated by IntrinsicCall.cpp (runtime calls or
inlined code with temporary allocation for array results). The fact that they
are the verbatim Fortran translations should allow to move the lowering code to a
translation pass without massive changes.
...
fir.elemental operation.

Syntax:
```
%res = fir.“char_op” %expr_or_var
```
...
For instance, for small array constructors, loops could could be unrolled with the array
ctor elements without ever creating a dynamically allocated array temporary and
loop nest using it.
...
element (they may contain a list of scalar and array values, which would render
the representation in a fir.elemental tricky, but maybe not impossible using
if/then/else and fir.elemental nests using the index value). One big issue
though is that fir.elemental requires the result shape to be pre-computed (it is
an operand), and with an ac-implied-do containing user transformational calls
returning allocatable or pointer arrays, it is impossible to pre-evaluate the
shape without evaluating all the function calls entirely (and therefore all the
array constructor elements).

#### fir.get_extent

Motivation: inquire about the extent of a fir.expr, variable, or fir.shape

Syntax:
```
%extent = fir.get_extent %shape_expr_or_var, dim
```
...
Motivation: inquire about the type parameters of a fir.expr, or variable.

Syntax:
```
%param = fir.get_typeparam %expr_or_var [, param_name]
```
- param_name is an optional string attribute that must contain the length
  parameter name if %expr_or_var is a derived type.

####  fir.get_dynamic_type

Motivation: inquire about the dynamic type of a polymorphic fir.expr or
variable.

Syntax:
```
%dynamic_type = fir.get_dynamic_type %expr_or_var
```
...
Syntax:
```
%shape = fir.shape_meet %shape1, %shape2
```
...
result storage allocation or vectorization of A+B.  fir.shape_meet intends to
...
The fir.forall syntax would be exactly the one of a fir.do_loop. The difference
would be that fir.assign and fir.ptr_assign inside fir.forall have specific
semantics (the same as in Fortran):
-   Given one fir.assign, all the iteration values of the LHS/RHS must be
    evaluated before the assignment of any value is done.
-   Given two fir.assign, the first fir.assign must be fully performed before
    any evaluation of the operands of the second assignment is done.
...
The translation of fir.forall will happen by:
-   1. Determining if the where masks value may be modified by any assignments
    ...
    - No, Do nothing (or indicate it is safe to evaluate masks while evaluating
      the rest).
-   2. Determining if a fir.assign operand expression depends on the
       previous fir.assign left-hand side base value.
    - Yes, split the fir.assign into their own nest of fir.forall loops.
...
box values that were produced by fir.declare, fir.associate, or fir.designate)
must have a fir.ref = mangled_name_attribute that matches the fir.def on the
operation that created them (it will be added automatically by the operation
builder). That is to ensure optimization passes do not merge seemingly identical
operations using variables with different properties, and also to ensure that
the matching defining operation can always be retrieved to get all the variable
properties (shape, bounds, type parameters and attributes).
...
-   Forall rewrites (getting rid of fir.forall)
-   Array assignment rewrites (getting rid of array fir.assign)
...
-   Call interface argument association lowering (getting rid of fir.associate
    and fir.end_associate)
-   Lowering high level operations using variables into FIR operations
    operating on memory (translating fir.designate, scalar fir.assign,
    fir.finalize into fir.array_coor, fir.do_loop, fir.store, fir.load.
...
-   Elemental expression inlining (inlining of fir.elemental in fir.apply)
-   User function Inlining
-   Transformational intrinsic rewrites as fir.elemental expressions
...
expression concepts described above should allow to keep an interface very similar
to the fir::ExtendedValue, but having the fir::ExtendedValue wrap a single value
or mlir::Operation* from which all of the object entity information can be
inferred.
...
- 1. Introduce the new FIR operations.
...
- 4. Introduce the fir.declare and fir.finalize usage in lowering (requires 1.
     and 2. and part of 3.).

The following steps might have to be done in parallel of the current lowering, to
avoid disturbing the work on performance until the new lowering is complete and
on par.

- 5. Introduce fir.designate and fir.associate usage in lowering.
- 6. Introduce lowering to fir.assign (with RHS that is not a fir.expr),
     fir.ptr_assign.
- 7. Introduce lowering to fir.expr and related operations.
- 8. Introduce lowering to fir.forall.
...
  fir.assign %b  to %a {fir.ref = "_QPfooEb,_QPfooEa"}: !fir.box<!fir.array<?xf32>>
  return
}
```

FIR array assignment lowering pass:
...
  %b = fir.declare  %arg1 {fir.def = "_QPfooEb"} : !fir.box<!fir.array<?xf32>>
  
  %ashape = fir.shape_of %a {fir.ref = "_QPfooEa"}
  %bshape = fir.shape_of %b {fir.ref = "_QPfooEb"}
  %shape = fir.shape_meet %ashape, %bshape
  %extent = fir.get_extent %shape, 0
  
  %c1 = arith.constant 1 : index
  
  fir.do_loop %i = %c1 to %extent step %c1 unordered {
     %belt = fir.designate %b, %i {fir.ref = "_QPfooEb", "fir.def=_QPfooEb.des001"}
     %aelt = fir.designate %a, %i {fir.ref = "_QPfooEa", "fir.def=_QPfooEa.des002"}
     fir.assign %belt  to %aelt {fir.ref = "_QPfooEb.des001,_QPfooEa.des002"}: fir.ref<f32>, fir.ref<f32>
  }
  return
}
```

Fir.var to memory translation pass:
-   fir.designate is rewritten into fir.array_coor operation on the variable
    associated memory buffer, and returns the element address
-   For numerical scalar, fir.assign is rewritten to fir.store (and fir.load of
    the operand if needed), for derived type and characters, memory copy (and
    padding for characters) is done.
-   fir.shape_of are lowered to fir.box_dims, here, no constant information was
    obtained from any of the source shape, so fir.shape_meet is a no-op,
...
This pass would wrap operations defining variables (fir.declare/fir.designate)
...
  %bshape = fir.shape_of %b {fir.ref = "_QPfooEb"}
  %pshape = fir.shape_of %p {fir.ref = "_QPfooEp"}
  %shape1 = fir.shape_meet %bshape, %pshape
  %mul = fir.elemental(%i:index) %shape1 {
    %belt = fir.designate %b, %i {fir.ref = "_QPfooEb", fir.def= "_QPfooEb.des001"}
    %p_lb = fir.get_lbound %p {fir.ref = "_QPfooEp"}
    %i_zero = arith.subi %i, %c1
    %i_p = arith.addi %i_zero,  %p_lb
    %pelt = fir.designate %p, %i_p {fir.ref = "_QPfooEp", fir.def= "_QPfooEp.des002"}
    %bval = fir.load %belt : f32
    %pval = fir.load %pelt : f32
    %mulres = arith.mulf %bval, %pval : f32
     fir.return %mulres : f32
  }
  %cshape = fir.shape_of %c
  %shape2 = fir.shape_meet %cshape, %shape1
  %add =  fir.elemental(%i:index) %shape2 {
     %mulval = fir.apply %mul, %i : f32
     %celt = fir.designate %c, %i {fir.ref = "_QPfooEc", fir.def= "_QPfooEc.des003"}
     %cval = fir.load %celt
     %add_res = arith.addf %mulval, %cval
     fir.return %add_res
  }
  fir.assign %add  to %a {fir.ref = "_QPfooEa"} : fir.expr<?xf32>, !fir.box<!fir.array<?xf32>
  return
}
```

Step 1: fir.elemental inlining: inline the first fir.elemental into the second
one at the fir.apply.
...
     fir.return %add_res
  }
  %extent = fir.get_extent %shape2, 0: (fir.shape<1>) -> index
...
  fir.assign %add  to %temp :  no_overlap  {fir.ref = "QPfoo.temp001"} : fir.expr<?xf32>, !fir.box<!fir.array<?xf32>
  fir.assign %temp  to %a :  no_overlap  {fir.ref = " QPfoo.temp001,_QPfooEa"} : fir.expr<?xf32>, !fir.box<!fir.array<?xf32>
  fir.finalize %temp {fir.ref = "QPfoo.temp001"}
...
attribute, and inline the fir.elemental into the first loop nest.
...
     %cval = fir.load %celt
     %add_res = arith.addf %mulval, %cval
     %tempelt = fir.designate %temp, %i {fir.ref = "_QPfoo.temp001", fir.def="_QPfoo.temp001.des004"}
     fir.assign %add_res  to %tempelt {fir.ref = "_QPfoo.temp001.des004"}: f32, fir.ref<f32>
  }
  fir.do_loop %i = %c1 to %shape2 step %c1 unordered {
     %aelt = fir.designate %a, %i {fir.ref = "_QPfooEa", fir.def= "_QPfooEa.des005"}
      %tempelt = fir.designate %temp, %i {fir.ref = "_QPfoo.temp001", fir.def="_QPfoo.temp001.des006"}
      fir.assign %add_res  to %tempelt {fir.ref = "_QPfoo.temp001.des005,_QPfooEa.des005"}: f32, fir.ref<f32>
  }
  fir.finalize %temp {fir.ref = "QPfoo.temp001"}
  fir.freemem %tempstorage
  return
...
-   %shape2 can be inferred from %cshape that has constant shape: the
    fir.shape_meet results can be replaced by it, and if the option is set,
...
Step 6: lower fir.designate/fir.assign in a translation pass:
...
- vector subscripted entities would be lowered as a fir.elemental implementing
...
- If the vector appears in a context where it can be modified (which can only
  ...
  fir.elemental into fir.forall (for assignments), or a fir.iter_while (for
  input IO) by inlining the elemental body into the created loops, and
  identifying the fir.designate producing the result.
...
  %v = fir.declare %arg2 {fir.def = "_QPfooEv"} : !fir.box<!fir.array<?xi32>>
  %vshape = fir.shape_of %v : fir.shape<1>
  %bsection =  fir.elemental(%i:index) %vshape : (fir.shape<1>) -> fir.expr<?xf32> {
    %v_elt = fir.designate %v, %i {fir.ref = "_QPfooEv", fir.def="_QPfooEv.des001"} : (!fir.box<!fir.array<?xi32>>, index) -> fir.ref<i32>
    %v_val = fir.load %v_elt : fir.ref<i32>
    %cast = fir.convert %v_val : (i32) -> index
    %b_elt = fir.designate %b, %v_val {fir.ref = "_QPfooEb", fir.def="_QPfooEb.des002"} : (!fir.ref<!fir.array<?xf32>>, index) -> fir.ref<f32>
    %b_val = fir.load %b_elt : fir.ref<f32>
    fir.result %b_elt
  }
  %extent = fir.get_extent %vshape, 0 : (fir.shape<1>) -> index
  %c1 = arith.constant 1 : index
  fir.forall (%i from %c1 to %extent step %c1) {
    %b_section_val = fir.apply %bsection, %i : (fir.expr<?xf32>, index) -> f32
    %v_elt = fir.designate %v, %i {fir.ref = "_QPfooEv", fir.def="_QPfooEv.des003"} : (!fir.box<!fir.array<?xi32>>, index) -> fir.ref<i32>
    %v_val = fir.load %v_elt : fir.ref<i32>
    %cast = fir.convert %v_val : (i32) -> index
    %a_elt = fir.designate %a, %v_val {fir.ref = "_QPfooEa", fir.def="_QPfooEa.des004"} : (!fir.ref<!fir.array<?xf32>>, index) -> fir.ref<f32>
    fir.assign %b_section_val to %a_elt {fir.ref="_QPfooEa.des004"} : f32, fir.ref<f32>
  }
  return
}
```

This would then be lowered as described in the examples above (fir.elemental
will be inlined, fir.forall will be rewritten into normal loops taking into
account the alias analysis, and fir.assign/fir.designate operations will be
...
rewriting fir.sum/fir.matmul to a linalg sum, with dialect types plumbing around
the operand and results, to get tiling done by linalg), using it as a lowering
target would not cover all Fortran needs (especially for the non semi affine cases).
So using linalg is for now left as an optimization pass opportunity in some cases
...
shape.meet operation is exactly similar with fir.shape_meet, except that it
...
## Using embox/rebox and box as an alternative to fir.declare/fir.designate and fir.expr/ variable concept
...
OPTIONAL or VOLATILE).  In that sense, it would be possible to have fir.declare,
fir.designate, and fir.associate be replaced by embox/rebox, and also to have
all operation creating fir.expr to create fir.box.
...
while a fir.expr value cannot change, which allows moving its usages more
easily.
...
fir.designate. Lowering will deal with the assignment and input IO special case
using fir.elemental.
