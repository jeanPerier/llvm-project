...
public:
  ...
  // unit (in case such conflicts exist, the behavior will depend of the linker).
  ...
  // handling the cases where the common block initialization is not the
  // appearance with the biggest size: the common block will have the biggest
  // size with the first bytes initialized to the initial value of the
  // appearance where the common block is initialized. (This is not standard,
  // if the initialization and biggest size appearances are in different
  // compilation units, the behavior will depend on the linker. The linker may
  // have the behavior described before, but it may also keep the initialized
  // common symbol without extending its size, or have some other behavior).
