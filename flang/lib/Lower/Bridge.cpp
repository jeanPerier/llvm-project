...
private:
  ...
  void genForallNest(const Fortran::parser::ConcurrentHeader &header) {
    ...
    auto evaluateControl = [&](const auto &parserExpr, mlir::Region &region,
      ...
      // region. They do not depend on any previous forall indices and no
      // assignment has been made yet that could modify their value. This
      // will simplify hlfir.forall analysis because the SSA integer
      // value yielded will obviously not depend on any variable modified by
      // the forall when produced outside of it.
  ...
  void genDataAssignment(
    ...
    auto evaluateRhs = [&](Fortran::lower::StatementContext &stmtCtx) {
      ...
      // dereferences pointer and allocatable RHS: the target is
      // being assigned from.
