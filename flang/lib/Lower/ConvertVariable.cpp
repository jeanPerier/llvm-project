...
static fir::GlobalOp getCommonBlockGlobal(Fortran::lower::AbstractConverter &converter, const Fortran::semantics::Symbol& common) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  if (!global)
    fir::emitFatalError(genLocation(converter, common), "COMMON block was not lowered before its usage");
  return global;
}
...
static std::optional<std::tuple<fir::GlobalOp, Fortran::semantics::MutableSymbolVector, mlir::Location>> declareCommonBlock(Fortran::lower::AbstractConverter &converter,
  ...
  mlir::Location loc = genLocation(converter, common);
  ...
  if (!commonBlockHasInit(cmnBlkMems)) {
    ...
    // No need to do any later definition
    return std::nullopt;
  }
...
/// an initial value.
static void
finalizeCommonBlockDefinition(mlir::Location loc, Fortran::lower::AbstractConverter &converter,
fir::GlobalOp global, const Fortran::semantics::MutableSymbolVector& cmnBlkMems) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
...
void Fortran::lower::defineCommonBlocks(Fortran::lower::AbstractConverter& converter, const Fortran::semantics::CommonBlockList& commonBlocks) {
  // Common blocks may depend on another common block address (if they contain pointers with initial targets). To cover this case, create all common block fir::Global before creating the initial values (if any).
  std::vector<std::tuple<fir::GlobalOp, Fortran::semantics::MutableSymbolVector, mlir::Location>> delayedInitializations;
...
static void instantiateCommon(Fortran::lower::AbstractConverter &converter,
  ...
  mlir::Location loc = genLocation(converter, varSym);
