...
static mlir::Value genInlinedStructureCtorLitImpl(
  ...
  for (const auto &[sym, expr] : ctor.values()) {
    ...
    assert(componentParentType_ && "failed to retrieve component parent type");
    if (!res) {
      ...
      // This component belong to a type extension of the type whose previous
      // component belonged to. A new undef for the type extension must be
      // created and the previous parent type value inserted into it. There may
      // be empty parent types in between the type whose current component
      // belong to and the previous one, hence the list and while loop.
      mlir::Type parentType = converter.genType(*componentParentType);
      llvm::SmallVector<mlir::Type> parentTypes = {parentType};
      while (true) {
