...
static constexpr MathOperation mathOperations[] = {
    ...
    // Cherry-picking changes for mlir::LLVM::RoundOp and mlir::math::AtanOp
    // into fir-dev is not feasible, so they are disabled for the time being.
...
IntrinsicLibrary::genIeeeIsFinite(mlir::Type resultType,
  ...
  mlir::Type intType =
      builder.getIntegerType(floatType.isa<Float80Type>() ? 128 : floatBits);
  mlir::Value intVal =
      builder.create<mlir::arith::BitcastOp>(loc, intType, floatVal);
  int significandBits;
  if (floatType.isa<Float32Type>())
    significandBits = 23;
  else if (floatType.isa<Float64Type>())
    significandBits = 52;
...
IntrinsicLibrary::genMerge(mlir::Type,
  ...
  auto rslt = builder.create<mlir::SelectOp>(loc, mask, tsource, fsourceCast);
  if (isCharRslt) {
