...
/// This class keeps track of the common block appearances with the biggest size and with an initial value (if any) in a program.
/// This allows complaining about conflicting initialization and warning about appearance of a same named common block with different sizes.
/// The biggest common block size and initialization (if any) can later be provided so that lowering can generate the correct symbol size
/// and initial values, even when named common blocks appears with different sizes and are initialized outside of block data.
...
private:
  struct CommonBlockInfo {
    ...
    // Common block symbol for the appearance with the initialized members (if any).
...
void MapCommonBlockAndCheckConflicts(SemanticsContext& context, const Symbol& common) {
  ...
  if (!firstAppearance) {
    ...
    if (isInitialized) {
      if (info.initialization.has_value() && &**info.initialization != &common) {
        // Use the location of the initialization in the error message because
        // common block symbols may have no location if they are blank commons.
        const Symbol& previousInit{DEREF(CommonBlockIsInitialized(**info.initialization))};
        context
            .Say(isInitialized->name(),
                "Multiple initialization of COMMON block /%s/"_err_en_US,
                common.name())
            .Attach(previousInit.name(), "Previous initialization of COMMON block /%s/"_en_US, common.name());
      } else {
        info.initialization = common;
      }
    }
    if (common.size() != info.biggestSize->size() && !common.name().empty()) {
      context.Say(common.name(), "A named COMMON block should have the same size everywhere it appears (%zd bytes here)"_port_en_US, common.size())
      .Attach(info.biggestSize->name(), "Previously defined with a size of %zd bytes"_en_US, info.biggestSize->size());
    }
    ...
    for (const Fortran::semantics::EquivalenceSet &set :
      ...
      for (const Fortran::semantics::EquivalenceObject &obj : set) {
        if (!obj.symbol.test(Fortran::semantics::Symbol::Flag::CompilerCreated)) {
          if (FindCommonBlockContaining(obj.symbol) == &common && IsInitialized(obj.symbol)) {
            return &obj.symbol;
          }
        }
      }
