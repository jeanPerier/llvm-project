...
struct ConcatOpConversion : public mlir::OpConversionPattern<hlfir::ConcatOp> {
  ...
  matchAndRewrite(hlfir::ConcatOp concat, OpAdaptor adaptor,
    ...
    mlir::Type addrType = fir::ReferenceType::get(hlfir::getFortranElementType(concat.getResult().getType()));
...
struct AssociateOpConversion : public mlir::OpConversionPattern<hlfir::AssociateOp> {
  ...
  matchAndRewrite(hlfir::AssociateOp associate, OpAdaptor adaptor,
    ...
    auto replaceWith = [&](mlir::Value hlfirVar, mlir::Value firVar, mlir::Value flag) {
    ...
    if (isTrivialValue) {
      ...
      fir::FirOpBuilder builder(rewriter, fir::getKindMapping(module));
      auto temp = builder.createTemporary(loc, bufferizedExpr.getType(), associate.getUniqName());
...
struct EndAssociateOpConversion : public mlir::OpConversionPattern<hlfir::EndAssociateOp> {
...
public:
  void runOnOperation() override {
    ...
    patterns.insert<AssignOpConversion, AssociateOpConversion, ConcatOpConversion, EndAssociateOpConversion>(context);
