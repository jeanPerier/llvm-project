...
/// Convert an entity to the same type as a given mold. This is intended to help
/// replacing block arguments with some hlfir entity that may be a value while
/// the block is a variable or vice-versa. These mismatch may occur between the
/// user defined assignment block arguments and the value that was lowered for
/// them which may be an in-memory copy while the block argument is an
/// hlfir.expr.
static hlfir::Entity
convertToMold(mlir::Location loc, fir::FirOpBuilder &builder,
  ...
  if (input.isVariable() && mold.isValue()) {
    if (fir::isa_trivial(mold.getType()))
      // fir.ref<T> to T.
      return hlfir::Entity{builder.create<fir::LoadOp>(loc, input)};
...
void OrderedAssignmentRewriter::pre(hlfir::RegionAssignOp regionAssignOp) {
  ...
  if (!regionAssignOp.getUserDefinedAssignment().empty()) {
    ...
    lhsEntity = convertToMold(loc, builder, lhsEntity, userAssignLhs,
                              argConversionCleanups);
    rhsEntity = convertToMold(loc, builder, rhsEntity, userAssignRhs,
                              argConversionCleanups);
