...
public:
  AllocaReplaceImpl(fir::AllocaRewriter allocaRewriter,
                    fir::DeallocGenerator deallocGenerator)
...
private:
  fir::AllocaRewriter allocaRewriter;
  fir::DeallocGenerator deallocGenerator;
...
static bool isRegionTerminator(mlir::Operation &terminator) {
  ...
  // has no results).
  // May be true for dead code. It is not a correctness issue and
  // dead code can be eliminated by running region simplification
  // before this utility is used.
  // FIXME remove this after end to end tests.
  if (!terminator.hasTrait<mlir::OpTrait::ReturnLike>())
    if (!terminator.hasSuccessors() &&
        !mlir::isa<mlir::omp::TerminatorOp>(&terminator))
      TODO(terminator.getLoc(), "unexpected terminator check");
...
bool AllocaReplaceImpl::replace(mlir::RewriterBase &rewriter,
  ...
  // FIXME: TODO is a for test only.
  if (!owningRegion) {
    llvm::errs() << "failed to reason about: " << alloca << "\n";
    TODO(alloca.getLoc(), "failed to reason about alloca owners");
  }
  // return false;
...
bool fir::replaceAllocas(mlir::RewriterBase &rewriter,
                         ...
                         AllocaRewriter allocaRewriter,
                         DeallocGenerator deallocGenerator) {
