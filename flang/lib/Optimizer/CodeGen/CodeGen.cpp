...
struct EmboxCommonConversion : public FIROpConversion<OP> {
  ...
  placeInMemoryIfNotGlobalInit(mlir::ConversionPatternRewriter &rewriter,
    ...
    auto alloca = this->genAllocaWithType(loc, boxPtrTy, defaultAlign, rewriter);
...
struct LoadOpConversion : public FIROpConversion<fir::LoadOp> {
  ...
  matchAndRewrite(fir::LoadOp load, OpAdaptor adaptor,
    ...
    // fir.ref<fir.box> is implemented as taking a snapshot of the descriptor
    // value into a descriptor temp.
    if (auto boxTy = load.getType().dyn_cast<fir::BoxType>()) {
      auto inputBoxStorage = adaptor.getOperands()[0];
      // Rewrite all uses ?
      // box_addr, box_dims ... fold at load.
      // array_coor: most likely cannot hoist/evaluate at def point/
      // make a copy if any store/call ops in-between ?
      // unknown: make a copy ?
      ...
      // fir.box of assumed rank and polymorphic entities does not have
      // as storage size that is know at compile time. The copy needs to
      // be runtime driven depending on the actual dynamic rank or type.
      ...
      auto newBoxStorage = genAllocaWithType(loc, boxPtrTy, defaultAlign, rewriter);
