...
static DynamicType BiggerType(DynamicType type) {
  // Use TypeOf to avoid hard coding REAL(2) = ieee 16 bits here.
  using Ieee16 =
  ...
  if (type.category() == TypeCategory::Real ||
    ...
    if (type.kind() == TypeOf<Ieee16>::kind ||
        type.kind() == TypeOf<Other16>::kind) {
      return {type.category(), TypeOf<Ieee32>::kind};
    }
  }
  return type;
}
