...
TEST_F(RuntimeCallTest, genArgumentValue) {
  ...
  mlir::Value result = fir::runtime::genArgumentValue(
      *firBuilder, loc, number, value, errmsg);
  checkCallOp(
      result.getDefiningOp(), "_FortranAArgumentValue", /*nbArgs=*/3, /*addLocArgs=*/false);
}
...
TEST_F(RuntimeCallTest, genArgumentLen) {
  ...
  mlir::Type intTy = firBuilder->getDefaultIntegerType();
  mlir::Value number = firBuilder->create<fir::UndefOp>(loc, intTy);
  mlir::Value result = fir::runtime::genArgumentLength(*firBuilder, loc, number);
  checkCallOp(result.getDefiningOp(), "_FortranAArgumentLength", /*nbArgs=*/1, /*addLocArgs=*/false);
}
...
TEST_F(RuntimeCallTest, genEnvVariableValue) {
  ...
  checkCallOp(result.getDefiningOp(), "_FortranAEnvVariableValue", /*nbArgs=*/4, /*addLocArgs=*/true);
}
...
TEST_F(RuntimeCallTest, genEnvVariableLength) {
  ...
  mlir::Value name = firBuilder->create<fir::UndefOp>(loc, charTy);
  mlir::Value trimName = firBuilder->create<fir::UndefOp>(loc, i1Ty);
  mlir::Value result = fir::runtime::genEnvVariableLength(
      *firBuilder, loc, name, trimName);
  checkCallOp(result.getDefiningOp(), "_FortranAEnvVariableLength", /*nbArgs=*/2, /*addLocArgs=*/true);
}
